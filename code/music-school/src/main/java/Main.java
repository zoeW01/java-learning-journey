...
public class Main{
	...
	public static void main(String[] args){
		...
		while(true){
			...
			if(choice == '1'){
				...
				System.out.println("正在执行根据乐器查询学生操作~");
	...
	public static void addStudent(){
		...
		Student student = new Student(sc.next(),sc.nextInt(),sc.next(),LocalDate.now(),sc.next());
	...
	public static void getStudentByInstrument(){
		try{
			...
			List<Student> students = StudentDao.getStudentByInstrument(sc.next());
	...
	public static void addPayment(){
		...
		Payment payment = new Payment(id,new BigDecimal(amount),LocalDate.now());
		boolean b = false;
	...
	public static void getPaymentsByStudentId(){
		try{
			...
			int id = sc.nextInt();
			List<Payment> payments = PaymentDao.getPaymentsByStudentId(id);
