...
| `contains(s)` | 是否包含子串 | `"hello".contains("ell")` → true |
| `startsWith(s)` | 以某串开头 | `"hello".startsWith("he")` → true |
| `endsWith(s)` | 以某串结尾 | `"hello".endsWith("lo")` → true |
| `replace(a, b)` | 替换所有匹配 | `"java".replace("a","o")` → "jovo" |
| `substring(i, j)` | 截取 [i, j-1] | `"hello".substring(0,2)` → "he" |
| `split(分隔符)` | 按分隔符切开 | `"a,b".split(",")` → `["a","b"]` |
| `charAt(i)` | 获取索引 i 的字符 | `"abc".charAt(0)` → 'a' |
